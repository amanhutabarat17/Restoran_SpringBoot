  ...
  <div class="admin-login-page">
    <div class="login-card">
      <div class="login-header">
        ...
        <h2>Admin RestoKu</h2>
        <p>Khusus untuk pengelola & staf resmi restoran</p>
      </div>

      <form class="login-form" @submit.prevent="submitLogin">
        <label class="field">
          <span class="field-label">Username / Email Admin</span>
          <input v-model="username" type="text" placeholder="admin@restoku.id" required />
        </label>
        ...
        <button type="submit" class="btn-submit" :disabled="sedangMasuk">
          {{ sedangMasuk ? 'Memproses...' : 'Masuk ke Dashboard' }}
        </button>
      </form>
...
const submitLogin = async () => {
  ...
  if (!username.value || !password.value) {
    pesanError.value = 'Mohon isi username dan kata sandi.'
    return
  }

  sedangMasuk.value = true
  const hasil = await login(username.value, password.value)
  sedangMasuk.value = false
  if (hasil.sukses) {
    router.push('/admin')
  } else {
    pesanError.value = hasil.pesan || 'Email atau kata sandi salah.'
  }
}
</script>
